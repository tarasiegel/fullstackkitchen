      ...
      sec.forEach((ref, refKey) => {
        if (updatedInstruction.indexOf(ref) !== -1) {
          ...
          updatedInstruction = updatedInstruction.replace(
            ref,
            `<span id="ref-${fullRefKey}" class="instruction-item" data-ref="${fullRefKey}" data-ingredient="${ref}" key="${refKey}" >${ref}</span>`
          );
        }
      });
    });
  ...
  render() {
    const { recipe, sectionKey } = this.props,
      ...
      ingredientHTML = recipe.ingredients.map((ing, key) => {
        return (
          <RecipeIngredients
            ...
            ingredientKey={key}
            strikeOutIngredient={this.strikeOutIngredient}
          />
        );
      }),
      directionsHTML = recipe.instructions.map((ins, key) => {
        return (
          <div
            className="recipe__instruction recipe__paragraph"
            index={key}
            key={key}
            dangerouslySetInnerHTML={{ __html: this.updateInstruction(ins, refData, sectionKey) }}
          />
        );
      }),
      assemblyHTML = recipe.assembly
        ? recipe.assembly.map((ass, key) => {
            return (
              <div
                className="recipe__assembly recipe__paragraph"
                index={key}
                key={key}
                dangerouslySetInnerHTML={{ __html: ass }}
              />
            );
          })
        : [];
...
class RecipeIngredients extends React.Component {
  ...
  render() {
    ...
    const ingHtml = ingredientData.data.map((ing, key) => {
      ...
      return (
        <li
          id={`ingredient--${referenceKey}`}
          key={key}
          className={`ingredient-section__ingredient-item ingredient--${referenceKey}`}
          data-name={ing.reference ? ing.reference : ing}
        >
          <input
            className="ingredient-section__ingredient-item-input"
            type="checkbox"
            id={`cbx-${referenceKey}`}
            data-ingredient={`${referenceKey}`}
    ...
    return (
      <div className="ingredient-section">
        ...
        <div className="ingredient-section__ingredients">{ingHtml}</div>
      </div>
    );
  }
}
