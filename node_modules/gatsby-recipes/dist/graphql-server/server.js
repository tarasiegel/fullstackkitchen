...
var html = entry.createCommonjsModule(function (module, exports) {
...
class Parser {
  ...
  _shouldProcessTokenInForeignContent(token) {
    ...
    const isMathMLTextStartTag = token.type === tokenizer.START_TAG_TOKEN && token.tagName !== $$4.MGLYPH && token.tagName !== $$4.MALIGNMARK;
...
var unistUtilPosition = entry.createCommonjsModule(function (module, exports) {
/* Expose. */
...
var html_1 = merge_1([xml, xlink, xmlns, aria, html$1]);

var boolean$2 = types.boolean;
...
var html$3 = factory_1(html_1, 'div');
...
var own$1 = {}.hasOwnProperty; // Handlers.
...
function transform$1(ast, config) {
  ...
  var fn = own$1.call(map, ast.nodeName) ? map[ast.nodeName] : element;
  var children;
  var node;
  var pos;
...
var hastUtilToParse5 = transform$2;
var ignoredSpaces = ['svg', 'html'];
var one = zwitch('type');
one.handlers.root = root$2;
one.handlers.element = element$2;
one.handlers.text = text$2;
one.handlers.comment = comment$1;
one.handlers.doctype = doctype$2; // Transform a tree from hast to Parse5’s AST.

function transform$2(tree, space) {
  return one(tree, space === 'svg' ? svg_1 : html_1);
}
...
var own$4 = {}.hasOwnProperty; // Transform an unknown node.
...
function one$1(h, node, parent) {
  ...
  var fn = own$4.call(h.handlers, type) ? h.handlers[type] : h.unknownHandler; // Fail on non-nodes.
...
function text$3(node) {
  ...
  if (own$4.call(data, 'hName') || own$4.call(data, 'hProperties') || own$4.call(data, 'hChildren')) {
    return false;
  }
...
var own$6 = {}.hasOwnProperty; // Characters
...
function encode$1(value, options) {
  ...
  function replace(char, pos, slice) {
    return one$2(char, slice.charCodeAt(pos + 1), settings);
  }
...
function one$2(char, next, options) {
...
function toNamed(name, next, omit, attribute) {
  ...
  if (omit && own$6.call(legacy, name) && dangerous.indexOf(name) === -1 && (!attribute || next && next !== equalsTo && !isAlphanumerical(next))) {
    return value;
  }
...
function indent(value) {
  var lines = value.split(lineFeed$2);
  var length = lines.length;
  var index = -1;
  var line;
  ...
  while (++index < length) {
    line = lines[index];
    lines[index] = content.test(line) ? before$1 + line : line;
  }

  return lines.join(lineFeed$2);
}
...
function mdxElement(node) {
  ...
  var content = this.all(node).join(block ? lineFeed$3 + lineFeed$3 : '');

  if (block && content) {
    content = lineFeed$3 + indent_1(content) + lineFeed$3;
  }
...
function serializeAttributes(nodes, block) {
  ...
  if (block && result.length !== 1) {
    return lineFeed$3 + indent_1(result.join(lineFeed$3)) + lineFeed$3;
  } // A span, or a block with a single attribute.


  return space$3 + dedentStart(indent_1(result.join(space$3)));
}
...
function mdxAttribute(node) {
  ...
  return name + equalsTo$1 + serializeValue(value);
}
...
var startsWithCapitalLetter_1 = startsWithCapitalLetter;
...
const {
  startsWithCapitalLetter: startsWithCapitalLetter$1
} = util;
      ...
      return {
        visitor: {
          JSXOpeningElement(path) {
            ...
            if (startsWithCapitalLetter$1(jsxName)) {
              names.push(jsxName);
...
var lib$2 = entry.createCommonjsModule(function (module, exports) {
...
MDXContent.isMDXComponent = true`; // Check JSX nodes against imports
  ...
  transformSync(importsAndExports, {
    configFile: false,
    babelrc: false,
    plugins: [lib$1, lib$2, babelPluginExtractImportNamesInstance.plugin, babelPluginExtractExportNamesInstance.plugin]
  });
  ...
  const fnPostMdxTypeProp = transformSync(fn, {
    configFile: false,
    babelrc: false,
    plugins: [lib$1, lib$2, babelPluginApplyMdxPropInstance.plugin]
  }).code;
  const exportStatementsPostMdxTypeProps = transformSync(exportStatements, {
    configFile: false,
    babelrc: false,
    plugins: [lib$1, lib$2, babelPluginApplyMdxPropToExportsInstance.plugin]
  }).code;
...
function render$1 (mdxSrc, cb, context, isApply, isStream = false) {
  const scopeKeys = Object.keys(scope);
  const scopeValues = Object.values(scope);
  const srcCode = transformMdx(mdxSrc);
  const component = new Function(...scopeKeys, transformCodeForEval(srcCode));
  const result = render(component(...scopeValues), cb, context, isApply, isStream);
  return result;
}

async function validateSteps(steps) {
  const errors = [];
  const firstStepPlan = await render$1(steps[0]);
...
async function createPlan(context, cb) {
  const result = await render$1(context.recipe, cb, context);
  return result;
}

function ApplyPlan(context, cb) {
  return render$1(context.recipe, cb, context, true, true);
}
...
const u$1 = unified__default['default']().use(toMDAST__default['default']).use(remarkStringify__default['default']).use(remarkMdx__default['default']).use(remarkMdxJs__default['default']);
...
const toMdx = nodes => {
  ...
  const mdxSrc = u$1.stringify(stepAst);
  return mdxSrc;
};

const parse$3 = async src => {
  const ast = u$1.parse(src);
...
const debug$1 = debugCtor__default['default'](`recipes-machine`);
  ...
  states: {
        ...
        onError: {
          ...
          actions: xstate.assign({
            error: (context, _event) => {
              debug$1(`error resolving recipe`);
              return {
    ...
    parsingRecipe: {
      invoke: {
        id: `parseRecipe`,
        src: async (context, _event) => {
          debug$1(`parsingRecipe`);
          const parsed = await parse$3(context.recipeSrc);
          debug$1(`parsedRecipe`);
          return parsed;
        },
        onError: {
          target: `doneError`,
    ...
    validateSteps: {
      invoke: {
        ...
        src: async (context, event) => {
          debug$1(`validatingSteps`);
          const result = await validateSteps(context.stepsAsMdx);

          if (result.length > 0) {
            debug$1(`errors found in validation`);
...
internals.buildFields = fields => {
  ...
  cache$1 = Object.create(null); // Empty cache
...
const internals$1 = {};
internals$1.configSchema = Joi.object().keys({
...
function composeType(schema, config = {}) {
  config = Joi.attempt(config, internals$1.configSchema);
...
function createTypes() {
  const resourceTypes = Object.entries(resources).map(([resourceName, resource]) => {
    ...
    const queryTypes = [];
    const mutationTypes = {};
    const joiSchema = Joi.object().keys({ ...resource.schema,
...
const schema$1 = new graphql_1.GraphQLSchema({
  query: rootQueryType,
